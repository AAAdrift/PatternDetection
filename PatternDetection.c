...
void output_alert(ATTACKPATTERN *pOnepattern,POnepOnepacket *pOnepacket)
{
...
{ 
   char patternfile[256];  //保存攻击模式文件名
	...
	if (readpattern(patternfile))
  	...
  	//phandle = pcap_open_live(device,200,1,500,errbuf);                    //打开设备 //phandle = pcap_open_live(device,200,1,500,errbuf);
  	
	phandle = pcap_open_offline("200412291400.dump", errbuf);
